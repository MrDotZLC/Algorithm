/*
 * @lc app=leetcode id=3603 lang=cpp
 *
...
public:
    // DP递归
    long long minCost1(int m, int n, vector<vector<int>>& waitCost) {
        ...
        auto dfs = [&](this auto &&dfs, int i, int j) -> long long {
            ...
            return ans = min(dfs(i - 1, j), dfs(i, j - 1)) + cost; 
        };
    ...
    long long minCost(int m, int n, vector<vector<int>>& waitCost) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                f[j + 1] = min(f[j], f[j + 1]) + waitCost[i][j] +
                           1LL * (i + 1) * (j + 1);
            }
        }
