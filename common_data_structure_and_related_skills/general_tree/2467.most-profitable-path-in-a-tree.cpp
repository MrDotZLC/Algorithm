...
public:
    int mostProfitablePath(vector<vector<int>>& edges, int bob, vector<int>& amount) {
        ...
        auto bob_dfs = [&](this auto &&bob_dfs, int x, int parent, int t) -> bool {
            if (x == 0) {
                bob_time[x] = t;
                return true;
            }
            for (auto &c : g[x]) {
                if (c != parent && bob_dfs(c, x, t + 1)) {
                    bob_time[x] = t;
                    return true;
                }
            }
        ...
        g[0].push_back(-1);
        int ans = INT_MIN;
        auto alice_dfs = [&](this auto &&alice_dfs, int x, int parent, int t, int total) -> void {
            if (t < bob_time[x]) {
                total += amount[x];
