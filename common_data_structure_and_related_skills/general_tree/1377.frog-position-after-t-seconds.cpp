...
public:
    // 自顶向下
    double frogPosition1(int n, vector<vector<int>>& edges, int t, int target) {
        ...
        auto dfs = [&](this auto &&dfs, int x, int parent, int left_t, long long prod) -> bool {
            ...
            for (auto &c : g[x]) {
                if (c != parent && dfs(c, x, left_t - 1, prod * (g[x].size() - 1))) {
                    return true;
                }
            }
            return false;
    ...
    double frogPosition(int n, vector<vector<int>>& edges, int t, int target) {
        ...
        auto dfs = [&](this auto &&dfs, int x, int parent, int left_t) -> long long {
            if (left_t == 0) {
                return x == target;
            }
            if (x == target) {
