...
public:
    vector<int> countPairsOfConnectableServers(vector<vector<int>>& edges, int signalSpeed) {
        ...
        auto dfs = [&](this auto &&dfs, int x, int parent, int sum_path) -> int {
            int cnt = sum_path % signalSpeed ? 0 : 1;
            for (auto &c : g[x]) {
                if (c.first != parent) {
                    cnt += dfs(c.first, x, sum_path + c.second);
                }
            }
            return cnt;
        };
        ...
        for (int i = 0; i < n; i++) {
            ...
            int s = 0;
            for (auto &c : g[i]) {
                int cnt = dfs(c.first, i, c.second);
                ans[i] += cnt * s;
                s += cnt;
            }
        }
