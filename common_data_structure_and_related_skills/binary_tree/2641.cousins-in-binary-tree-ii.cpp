...
public:
    TreeNode* replaceValueInTree(TreeNode* root) {
        ...
        while (!q.empty()) {
            vector<pair<TreeNode*, TreeNode*>> t;
            unordered_map<TreeNode*, int> parent_mp;
            int i = q.size(), sum = 0;
            while (i--) {
                auto [node, parent] = q.front();
                q.pop_front();
                t.push_back({node, parent});
                sum += node->val;
                if (parent) {
                    parent_mp[parent] += node->val;
                }
                if (node->left) {
                    q.push_back({node->left, node});
                }
                if (node->right) {
                    q.push_back({node->right, node});
                }
            }

            for (auto &[node, parent] : t) {
                node->val = parent ? sum - parent_mp[parent] : 0;
            }
        }
        return root;
    }
