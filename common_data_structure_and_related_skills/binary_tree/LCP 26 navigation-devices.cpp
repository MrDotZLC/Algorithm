...
public:
    int navigation(TreeNode* root) {
        int ans = 0;
        bool has_single_chain = 1;
        auto dfs = [&](auto &dfs, TreeNode *node) -> bool {
            if (!node) {
                return false;
            }
            bool l = dfs(dfs, node->left);
            bool r = dfs(dfs, node->right);
            if (node->left && node->right) {
                ans += !l && !r;
                has_single_chain = !(l && r);
                return true;
            }
            return l || r;
        };
        bool l = dfs(dfs, root->left);
        bool r = dfs(dfs, root->right);
        return ans + (l && r ? 0 : has_single_chain);
    }
};
// @lc code=end
