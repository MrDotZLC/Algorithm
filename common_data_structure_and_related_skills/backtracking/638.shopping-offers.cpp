...
public:
    int shoppingOffers(vector<int>& price, vector<vector<int>>& special, vector<int>& needs) {
        ...
        auto dfs = [&](auto &dfs, vector<int> cur_needs) -> int {
            string key;
            for (int x : cur_needs) {
                key += x + 'a';
            }
            ...
            int sum = 0;
            for (int i = 0; i < n; i++) {
                sum += price[i] * cur_needs[i];
            }

            for (int i = 0; i < special.size(); i++) {
                bool can_buy = true;
                for (int j = 0; j < n; j++) {
                    if (special[i][j] > cur_needs[j]) {
                        can_buy = false;
                        break;
                    }
                }
                ...
                if (can_buy) {
                    vector<int> next_needs = cur_needs;
                    for (int j = 0; j < n; j++) {
                        next_needs[j] -= special[i][j];
                    }
                    sum = min(sum, special[i][n] + dfs(dfs, next_needs));
                }
            }
            return memo[key] = sum;
        };
