...
public:
    int perfectMenu(vector<int>& materials, vector<vector<int>>& cookbooks, vector<vector<int>>& attribute, int limit) {
        ...
        int ans = 0;
        auto dfs = [&](this auto &&dfs, int i, int taste, int satiety, vector<int> &leave_m) -> void {
            if (i == n) {
                if (satiety >= limit) {
                    ans = max(ans, taste);
                }
            ...
            dfs(i + 1, taste, satiety, leave_m);

            bool can_cooking = true;
            for (int j = 0; j < cookbooks[i].size(); j++) {
                if (leave_m[j] < cookbooks[i][j]) {
                    can_cooking = false;
                }
                leave_m[j] -= cookbooks[i][j];
            }
            if (can_cooking) {
                dfs(i + 1, taste + attribute[i][0], satiety + attribute[i][1], leave_m);
            }
            for (int j = 0; j < cookbooks[i].size(); j++) {
                leave_m[j] += cookbooks[i][j];
            }
